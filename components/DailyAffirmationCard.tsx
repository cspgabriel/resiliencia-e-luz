...
const DailyAffirmationCard: React.FC<Props> = ({ settings, onXpGain }) => {
  ...
  const handleShare = async () => {
    ...
    setBusy(true);
    try {
      const blob = await renderCard({ title: 'Para hoje', body: aff.text, emoji: '🌟', palette: [aff.palette[0], aff.palette[1]], footer: today() });
      const r = await shareOrDownload(blob, `resilienciaeluz-${today()}.png`, aff.text);
  ...
  return (
    <div className="rounded-3xl p-5 shadow-sm border border-white/40" style={{ background: `linear-gradient(135deg, ${aff.palette[0]}, ${aff.palette[1]})` }}>
