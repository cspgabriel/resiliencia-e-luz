...
const Paywall: React.FC<Props> = ({ onBack, onSubscribe, reason }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-violet-500 via-fuchsia-500 to-pink-500 text-white pb-24">
      ...
      <div className="max-w-md mx-auto px-5 pt-4">
        ...
        <div className="space-y-3 mb-6">
          {[
            { icon: MessageCircle, text: 'Chat ilimitado com Sereno' },
        ...
        <div className="space-y-3 mb-6">
          <button
            ...
            className={`w-full text-left rounded-2xl p-4 border-2 transition ${plan === 'yearly' ? 'bg-white text-slate-900 border-white' : 'bg-white/10 border-white/30'}`}
          >
            <div className="flex justify-between items-start">
              <div>
                <p className="font-bold">Anual</p>
                <p className="text-xs opacity-80">R$ 9,90/mês cobrado anualmente</p>
              </div>
              <div className="text-right">
                <p className="font-bold">R$ {PRICING.yearly.price.toFixed(2).replace('.', ',')}</p>
                <span className="text-[10px] font-bold bg-emerald-500 text-white px-2 py-0.5 rounded-full">{PRICING.yearly.savings}</span>
              </div>
            </div>
          </button>
          ...
          <button
            ...
            className={`w-full text-left rounded-2xl p-4 border-2 transition ${plan === 'monthly' ? 'bg-white text-slate-900 border-white' : 'bg-white/10 border-white/30'}`}
          >
            <div className="flex justify-between items-center">
              <div>
                <p className="font-bold">Mensal</p>
                <p className="text-xs opacity-80">Cancela quando quiser</p>
              </div>
        ...
        <button
          ...
          className="w-full bg-white text-violet-700 font-bold py-4 rounded-2xl shadow-2xl flex items-center justify-center gap-2"
        >
          <Check className="w-5 h-5" /> Assinar Plus
        </button>

        <p className="text-xs text-center text-white/80 mt-4">
          Pagamento seguro via Stripe ou Mercado Pago.<br />Cancele a qualquer momento, sem multa.
        </p>
      </div>
    </div>
  );
