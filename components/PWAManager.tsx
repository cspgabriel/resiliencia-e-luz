...
const DISMISS_KEY = 'serenamente_pwa_dismissed_at';
...
const PWAManager: React.FC = () => {
  ...
  // Registra o SW
  useEffect(() => {
    const fn = registerPwa({
      onNeedRefresh: () => setUpdateReady(true),
      onOfflineReady: () => { setOfflineReady(true); setTimeout(() => setOfflineReady(false), 4000); },
    });
    if (fn) setRefreshFn(() => fn);
  }, []);

  // Install prompt (Android/desktop Chrome)
  useEffect(() => {
    const unsub = onInstallAvailable((avail) => {
      setInstallAvailable(avail);
      if (avail && !isStandalone() && !wasRecentlyDismissed()) {
        // Mostra após 30s pra não ser invasivo no primeiro uso
  ...
  // iOS: não tem beforeinstallprompt — mostra hint manual
  useEffect(() => {
    if (!isiOS() || isStandalone() || wasRecentlyDismissed()) return;
    const t = setTimeout(() => setShowIOSHint(true), 45000);
  ...
  const handleInstall = async () => {
    trackSafeEvent('pwa_install_clicked');
    const result = await triggerInstall();
    trackSafeEvent('pwa_install_result', { result });
  ...
  const handleDismiss = () => {
    ...
    trackSafeEvent('pwa_install_dismissed');
  };

  const handleRefresh = async () => {
    trackSafeEvent('pwa_update_applied');
  ...
  return (
    <>
      {/* OFFLINE INDICATOR */}
      {!online && (
        <div className="fixed top-2 left-1/2 -translate-x-1/2 z-[60] bg-amber-500 text-white px-3 py-1.5 rounded-full text-xs font-semibold flex items-center gap-1.5 shadow-lg">
          <WifiOff className="w-3 h-3" /> Modo offline · seus dados ficam salvos
        </div>
      )}

      {/* OFFLINE READY */}
      {offlineReady && (
        <div className="fixed bottom-24 md:bottom-6 left-1/2 -translate-x-1/2 z-[60] bg-emerald-500 text-white px-4 py-2.5 rounded-2xl shadow-lg text-sm flex items-center gap-2">
      ...
      {updateReady && (
        <div className="fixed bottom-4 left-4 right-4 md:left-auto md:right-4 md:max-w-sm z-[60] bg-slate-900 dark:bg-slate-800 text-white rounded-2xl shadow-2xl p-4 flex items-center gap-3 border border-slate-700">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs opacity-80">Atualize para receber as melhorias.</p>
          </div>
          <button onClick={handleRefresh} className="px-3 py-2 bg-emerald-500 rounded-xl text-xs font-semibold">Atualizar</button>
        ...
        <div className="fixed bottom-20 md:bottom-6 left-4 right-4 md:left-auto md:right-4 md:max-w-sm z-[55] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-slate-500 mt-0.5">Acesse mais rápido, mesmo offline. Sem ocupar muito espaço.</p>
              <div className="flex gap-2 mt-3">
                <button onClick={handleInstall} className="flex-1 py-2 bg-emerald-500 text-white rounded-xl text-xs font-semibold">Instalar</button>
      ...
      {/* iOS HINT (não tem beforeinstallprompt) */}
      {showIOSHint && (
        <div className="fixed bottom-20 left-4 right-4 z-[55] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="font-semibold text-slate-900 dark:text-white text-sm">Adicionar à tela inicial do iPhone</p>
